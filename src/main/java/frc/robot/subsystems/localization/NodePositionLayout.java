 ...
 * used to change the poses returned from {@link NodePositionLayout#getTagPose(int)} to be from the
...
public class NodePositionLayout {
  ...
  private NodePositionLayout(
      @JsonProperty(required = true, value = "tags") List<NodePosition> nodePositions) {
   ...
   */
  @JsonProperty("tags")
  public List<NodePosition> getTags() {
  ...
  public Optional<Pose3d> getTagPose(int ID) {
    NodePosition tag = m_nodePositions.get(ID);
    if (tag == null) {
